...
void i2c1_init(void) {
    ...
    uint32_t pclk1 = SystemCoreClock / 2; // Обычно APB1 = 36 МГц, если SystemCoreClock = 72 МГц

    I2C1->CR2  = (pclk1 / 1000000);     // APB1 freq в МГц

    // --- Для 400 кГц ---
    uint16_t ccr = pclk1 / (3 * 400000); // Fast mode, duty=0 (2:1)
    I2C1->CCR  = (ccr & 0xFFF) | (1 << 15); // CCR[15]=1 (FS=Fast mode)
    I2C1->TRISE = (pclk1 / 1000000) * 300 / 1000 + 1; // 300нс
